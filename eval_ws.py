...
def get_opts():
    ...
    parser.add_argument('--img_width', type=int, default=200,
                        help='output width for the wikiscenes dataset')
...
if __name__ == "__main__":
    ...
    # elif args.dataset_name == 'wikiscenes':
    #     kwargs['use_cache'] = args.use_cache
    #     kwargs['img_width'] = args.img_width
    else:
    ...
    if (args.dataset_name == 'phototourism' or args.dataset_name == 'wikiscenes') and args.split == 'test':
        ...
        if scene == 'brandenburg_gate':
            ...
            img = Image.open(os.path.join(args.root_dir, 'dense/images',
                                          dataset.image_paths[dataset.img_ids_train[100]])).convert('RGB')  # 111 159 178 208 252 314
            img_downscale = 8
            ...
            # dz = np.linspace(0, 0.5, N_frames)
            # # define poses
            # dataset.poses_test = np.tile(dataset.poses_dict[10], (N_frames, 1, 1))
            # for i in range(N_frames):
            #     dataset.poses_test[i, 0, 3] += dx[i]
            #     dataset.poses_test[i, 1, 3] += dy[i]
            #     dataset.poses_test[i, 2, 3] += dz[i]
    ...
    for i in tqdm(range(len(dataset))):
        ...
        rays = sample['rays']
        ts = sample['ts']
        # if args.split == 'test' and args.encode_a:
        #     whole_img = sample['whole_img'].unsqueeze(0).cuda()
        #     kwargs['a_embedded_from_img'] = embedding_a(whole_img)
    ...
    if args.dataset_name == 'blender' or \
            ((args.dataset_name == 'phototourism' or args.dataset_name == 'wikiscenes') and args.split == 'test'):
